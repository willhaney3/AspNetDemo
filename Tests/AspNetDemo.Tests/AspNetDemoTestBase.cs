...
namespace AspNetDemo.Tests
{
    public abstract class AspNetDemoTestBase : AbpIntegratedTestBase<AspNetDemoTestModule>
    {
        ...
        protected void UsingDbContext(Action<AspNetDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AspNetDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AspNetDemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AspNetDemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AspNetDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AspNetDemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AspNetDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AspNetDemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AspNetDemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AspNetDemoDbContext, Task<T>> func)
        {
            T result;
